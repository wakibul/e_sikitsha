  ...
  <div class="row">
    <div class="col-12">
      <div class="page-header">
        <h1 class="page-title">
          Add a note for <b>{{strtoupper(strtolower($doctor->name))}}</b>
        </h1>
      </div>
      <div class="card-body">
        <div class="row">
        ...
        <div class="row">
          <div class="col-md-12">
        <form class="form-horizontal" action="{{route('admin.doctor.add_note.store')}}" method="POST">
            @csrf
              <label>Remarks</label>
              <input type="hidden" name="doctor_id" value="{{$doctor->id}}">
              <textarea name="notes" class="form-control" rows="5" required="true"></textarea>
              <button type="submit" class="btn btn-success">Add to Doctor Desk</button>
        </form>
          </div>
        </div>
      </div>
